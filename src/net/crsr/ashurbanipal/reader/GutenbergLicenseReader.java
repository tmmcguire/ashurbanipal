...
public class GutenbergLicenseReader extends Reader {
	...
	/**
	 * Attempt to skip the Project Gutenberg file header and footer, only returning the text contents of the file.
	 */
	public GutenbergLicenseReader(Reader reader) throws IOException {
		try {
			...
			//		System.out.println(suffix[0].length() + " - " + prefix[0].length() + " - " + (prefix.length > 1 ? prefix[1].length() : 0));
			string = prefix[0];
		} finally {
			reader.close();
		}
