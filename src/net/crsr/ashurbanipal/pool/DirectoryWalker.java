...
public class DirectoryWalker {
	...
	public int walk(String directory) {
		...
		// walk the directory hierarchy adding all unseen .zip files to the to-do list
		...
		while (!queue.isEmpty()) {
			...
			final File[] zipFiles = current.listFiles( new FileFilter() {
				@Override public boolean accept(File f) { return f.isFile() && f.getName().endsWith(".zip"); }
			});
			for (File f : zipFiles) {
		...
		// (handling shorter files should keep the tagger from running out of memory as long as possible) 
		todoList.sort(new Comparator<File>() {
			@Override public int compare(File left, File right) { return Long.compare(left.length(), right.length()); }
		});
